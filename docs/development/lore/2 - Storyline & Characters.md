...
- **Kingdom storyline (L5–36/37):** Main quest — the artifact search, lore, the twist
    - Each kingdom leader has their own motivations regarding the truth (see [[1 - World Lore & Kingdoms#Kingdom Rulers]])
...
**The Oracle's portrait:** Brad will create a sprite for the Oracle.
...
- The 25 endgame pieces are the hardest to collect (see [[4 - Game Systems#Collection System]])
...
> **Discussion Needed:** The title stat boosts are a newer idea. Need to flesh out exact numerical values and ensure they don't create balance issues at endgame. Should these be flat bonuses or percentage-based?
...
**The player names this character** — the first time the rival appears (~L8), the player is prompted to give them a name. **Text input modal, alphabetical characters only** (like naming your rival in Pokémon).
...
### Storyline
1. **Q: What is the final assembled artifact?**
...
2. **Q: Is The Choice permanent?**
   A: Yes. Restart the game for a different path.

3. **Q: Are the endgame paths mechanically different?**
...
### The Rival
4. **Q: What class is the rival?**
   A: Same class as the player, 3 levels higher. Creates a mirror-match dynamic.

5. **Q: Does the rival have a squad?**
   A: No — solo operative for now. Squad encounters are a future feature (requires party system).

6. **Q: Can the player name the rival?**
   A: Yes — text input modal at the first encounter (~L8), alphabetical characters only.

7. **Q: Does the Oracle need a visual portrait or just an emoji?**
   A: Brad will create a sprite for the Oracle.
