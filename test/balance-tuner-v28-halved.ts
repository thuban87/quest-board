/**
 * Combat Balance v28 - HALVED GEAR TEST
 * 
 * Tests the proposed fix: halving all gear stats
 * Formula: primaryValue = (level * 1.5) + (tier * level * 0.5)
 * 
 * Run: npx tsx test/balance-tuner-v28-halved.ts
 */
...
let output = `# Combat Balance v28 - HALVED GEAR TEST\n\n`;
output += `Generated: ${new Date().toISOString()}\n\n`;
output += `## Formula Change\n\n`;
output += `**Before:** \`primaryValue = (level * 3) + (tier * level)\`\n\n`;
output += `**After:** \`primaryValue = (level * 1.5) + (tier * level * 0.5)\`\n\n`;
output += `This halves all gear stats (attack, defense, magic defense).\n\n`;

output += `## Overworld (Target: 50-80%)\n\n`;
...
output += `\n---\n*Halved gear formula test*\n`;

const outputPath = path.join(__dirname, 'balance-results-v28-halved.md');
